...
async function app(req,res){
  ...
  if(req.method==='POST'&&u.pathname==='/api/auth/login'){const b=await readBody(req),email=String(b.email||'').trim().toLowerCase();if(!/^\S+@\S+\.\S+$/.test(email)||b.code!=='123456')return send(res,401,{error:'邮箱或验证码错误'});const user={email,role:'user'};const token=sessionToken(user);sessions.set(token,user);const isNew=!d.users[email];d.users[email]??={email,nickname:email.split('@')[0],bio:'正在积累 AI 能力'};await save(d);if(isNew)await sendEmail(email,'欢迎加入 AI Bloom',welcome(d.users[email]),'welcome:'+email,d);return send(res,200,{token,user:d.users[email]})}
  ...
  if(req.method==='PUT'&&u.pathname==='/api/profile'){const b=await readBody(req);d.users[me.email]={...(d.users[me.email]||{}),nickname:b.nickname||'',bio:b.bio||'',email:me.email};await save(d);return send(res,200,{user:d.users[me.email]})}
