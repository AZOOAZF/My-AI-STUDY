...
async function app(req,res){
  ...
  if(req.method==='POST'&&u.pathname==='/api/quiz-results'){
    const b=await readBody(req),day=Number(b.day),paper=Array.isArray(b.answers)?b.answers:[];if(!Number.isInteger(day)||day<1||day>56||paper.length!==100)return send(res,400,{error:'试卷必须提交完整的 100 道题'});
    const seen=new Set(),counts={fill:0,choice:0,response:0};let correct=0;for(const item of paper){const type=String(item.type||''),id=String(item.id||''),sourceDay=Number(item.sourceDay),task=d.tasks.find(taskItem=>taskItem.day===sourceDay),value=String(item.value||'').trim();if(!task||!/^((fill|choice|response)-\d+)$/.test(id)||seen.has(id)||!Object.prototype.hasOwnProperty.call(counts,type))return send(res,400,{error:'试卷题目数据无效'});seen.add(id);counts[type]++;if(type==='fill'||type==='choice'){if(quizAnswer(value)===quizAnswer(task.answer))correct++;}else if(value)correct++;}
    if(counts.fill!==34||counts.choice!==33||counts.response!==33)return send(res,400,{error:'试卷题型数量必须为 34/33/33'});
    d.quizResults[me.email]??={};d.quizResults[me.email][String(day)]={day,score:correct,total:100,correct,counts,submittedAt:new Date().toISOString()};await save(d);return send(res,200,{result:d.quizResults[me.email][String(day)]});
  }
